...
function _initPreloader() {
    ...
    $doc.on('click', 'a:not(.no-fade):not(.nk-video-fullscreen-toggle):not(.nk-gallery-item):not([target="_blank"]):not(.btn):not(.button):not(.nk-btn):not([href^="#"]):not([href^="mailto"]):not([href^="javascript:"])', function (e) {
...
function _initPluginJarallax() {
    ...
    if ($parallaxHeader.length) {
        (function () {
            var $content = $parallaxHeader.find('> .bg-image');
            ...
            $parallaxHeader.jarallax({
                ...
                onScroll: function onScroll(calc) {
                    if (headerParallaxOpacity) {
                        ...
                        $content.css({
                            opacity: calc.beforeTop > 0 ? 1 : percent
                        });
                    }
                }
            });
    ...
    if ($parallaxFooter.length) {
        (function () {
            var $content = $parallaxFooter.find('> .bg-image');
